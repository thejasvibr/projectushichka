# -*- coding: utf-8 -*-
"""
Aligning audio for 2018-07-14 speaker playbacks
===============================================
...
24 channels:
    ...
    * Channel 8-11: empty channels
    * Channel 12: poor mic connection on device 2
    * Channel 13-17 : microphones on device 2
    * Channel 14: no mic
...
recording_timestamp = '2018-07-15_07-39-16' # 2 mins long, 16 audio files at 192 kHz
filenames = glob.glob('./'+'SPKR*'+recording_timestamp+'*')

#%% Load the sync signals only and compare the delay between the two 
partaudio, fs = sf.read(filenames[0], stop=192000)
sync1 = partaudio[:,7]
sync2 = partaudio[:,19]
...
with open('../mic2mic_measurements/2018-07-14_goodmics.txt','r') as f:
...
# goodmics += [7,19] -- to  check that the sync signals are actually getting aligned 
...
sf.write('digital_playbacksignal_2018-07-14.WAV', output_signal, fs)
