...
def make_v2_playback_sounds():
    ...
    frequency_step = 1000;
    freqs = np.arange(pulse_start_f, pulse_end_f, frequency_step)
    pulse_zeros = np.zeros(int(common_length*fs)-int(pulse_durn*fs))
    left_pulse_silence, right_pulse_silence = np.array_split(pulse_zeros, 2)
    all_freq_pulses = []

    for each_freq in freqs:
        one_tone = np.sin(2*np.pi*t_pulse*each_freq)
        one_tone *= signal.tukey(one_tone.size, 0.95)
        one_tone *= 0.5
...
def perform_v2_playback(playback_sounds, mic_rec_name, fs=192000, dev_ind=40, **kwargs):
    '''
    '''
    num_in_out = kwargs.get('num_in_out',[9,1])
    data_in_channel = kwargs.get('data_in_channel',[8])
...
# fs = 192000
# audio_pbk, _ = make_v2_playback_sounds()
# sweep_flat = np.concatenate(audio_pbk[0])
# tones_flat = np.concatenate(audio_pbk[1])
# all_audio = np.concatenate((sweep_flat, tones_flat))
# WAV.write('pbk_v2.wav', int(fs),all_audio)
    
    playback_sounds, numsamples_comlength = make_v2_playback_sounds()
    mic_num = 'gras-2mwall_amp401_speaker504'
    angle = '90'
    gain = '50'
    orientation='azimuth'
    kwargs = {'save_path':'../'}
