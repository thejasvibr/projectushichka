# -*- coding: utf-8 -*-
"""
How reliable is the alignment? Part 3
=====================================
Julian Jandeleit (JJ) worked on estimating the pose and location of three 
thermal cameras in a LiDAR scan of the Orlova Chuka cave using keypoints 
inside the cave. 

The cave had microphones placed on the walls and an array-frame, all of which
were captured on the LiDAR scan. Here I visualise the 'fit' of the 
camera placement in the cave by plotting the 3D mic locations in the cave.
...
B = np.loadtxt('v2_transformation.csv', delimiter=',') # round 2 post ICP
...
# Open a movie file
plotter.open_movie('thermo-lidar_alignment_v2.mp4', framerate=25, quality=4)
...
#%%
start_camera_pos = (3.7123199465845342,
                    0.19909826300124092,
                    -1.041653569330262)
end_camera_pos = (2.8658470324818657,
                 -1.7164564338765609,
                 -1.6956853891383652)
...
camera_roll = (-89, -100)
camera_elev = (0.5, 0.5)
fpoint_start = (-3.749681192709187,
                2.752914417889359,
                0.27757022381482277)
fpoint_end =   (-3.9629910869837053,
                2.1443213882625427,
                -0.133988232544467)
