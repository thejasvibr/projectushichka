...
def create_new_track(first_frame, first_position, pos_index, class_label=None, head_position=None, noise=10,
    ...
    new_track = {'track': [first_position],
                ...
                'contour': [contour],
                 'size': [size],
                 'debug': ['{} created'.format(first_frame)],
...
def update_unmatched_track(track, debug=False):
    ''' Update track for next frame when there is no new point to add.
        ...
        track never matched before than noise value increases.
...
def filter_bad_assigns(track_list, active_list, distance, max_distance, row_ind, col_ind, double_assign=False, debug=False):
...
def process_points_without_tracks(distance, max_distance, track_list, 
                                  new_positions, contours, frame_ind, 
                                  sizes):
        ...
        for ind in new_track_ind:
            ...
            track_list.append(
                create_new_track(first_frame=frame_ind, 
                                 ...
                                 pos_index=ind, noise=1, 
                                 contour=contours[ind], size=sizes[ind]
                                )
            )
...
def remove_noisy_tracks(track_list):
    ...
    for track_num in range(len(track_list) - 1, -1, -1): 
        if track_list[track_num]['noise'] >= 2:
...
def create_tracks_for_leftover_points(track_list, col_inds, frame_ind, 
    ...
    for pos_ind in range(new_positions.shape[0]):
        ...
        # This just a conservative choise in case of an object 
        ...
        if np.min(distance[:, pos_ind]) > min_new_track_distance:
            ...
            track_list.append(
                create_new_track(first_frame=frame_ind, 
                                 ...
                                 pos_index=new_position_indexes[pos_ind], noise=1, 
...
def update_tracks(track_list, active_list, frame_index, row_inds, col_inds,
                  new_positions, new_position_indexes, new_sizes,
                  new_contours, distance, min_new_track_distance, debug=False):
