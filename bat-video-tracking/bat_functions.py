...
def subtract_background_ben(images, image_ind, background_sum):
    '''
    Ben's original implementation - doesn't work so well when the number of 
    images increase beyond ~70 for Ushichka data.
    ...
    ----------
    images : np.array  
        3d numpy array (num images, height, width)
    image_ind: int
        Index of the image to be background subtracted. 
        (orig: 'Index in circular image array')
    background_sum: np.array
        Sum of blue channel pixels across 0 dimension of images
       
    Returns
    -------
    image_dif, background : np.array   
        2D array of shape as one of the images
    '''
...
def subtract_background(images, image_ind, dummy_entry=0):
    '''
    TB's implementation of Ben's function - treats median pixel value as the 
    'background' over which subtraction takes place. 

    Subtract current imagge from the median background of the stack
    ...
    image_ind: int
        Index of the image to be background subtracted. 
        (orig: 'Index in circular image array')
    dummy_entry: MxN np.array, optional 
        dummy variable here to keep code compatibility - can be ignored.
    ...
    background = np.median(images[:,:,:,2], axis=0)
...
def preprocess_to_binary(image, binary_thresh, background):
    ...
    binary_thresh : float?
        ????
    background: np.array 2D
        The mean intensity value for all pixels in a position across 
        images.
    '''
...
def process_frame(images, focal_frame_ind, bat_thresh, background_sum, bat_area_thresh, debug=False):
    ...
    Notes 
    -----
    Has a constant 'max_bats' - the maximum number of bat centres that can be detected in an 
    image. 
...
# def measure_crossing_bats(track_list, frame_height=None, frame_width=None,
    ...
    # if count_out:
        ...
        # out_line = (int(frame_height/2), line_dim=1, total_frames=num_frames)
