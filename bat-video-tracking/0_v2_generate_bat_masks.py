...
------------------------
Step 1: De-noise the fixed pattern noise and generate 'clean' images and save them.
Step 2: Calculate the entropy profile of the cleaned image
Step 3: Threshold the entropy profile of the image and save the binary masks.
"""
import argparse
...
for num, img in tqdm.tqdm(enumerate(cleaned_images)):
    ...
    entropy_profile = rank.entropy(img, footprint)
    smoothed_stack.append(filtered_img)
    entropy_stack.append(entropy_profile)
...
background_image = np.apply_along_axis(np.median, 0, entropy_stack)
bground_subtr = entropy_stack - background_image
